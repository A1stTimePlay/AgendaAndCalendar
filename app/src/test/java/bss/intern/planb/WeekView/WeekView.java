...
import static bss.intern.planb.WeekView.WeekViewUtil.daysBetween;
import static bss.intern.planb.WeekView.WeekViewUtil.getPassedMinutesInDay;
import static bss.intern.planb.WeekView.WeekViewUtil.isSameDay;
import static bss.intern.planb.WeekView.WeekViewUtil.today;
            ...
            if ((mEmptyViewClickListener != null || mAddEventClickListener != null) && e.getX() > mHeaderColumnWidth && e.getY() > (mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)) {
                ...
                if (selectedTime != null) {
                    ...
                    if (mAddEventClickListener != null) {
                        ...
                        // Bug: add multiple new new-event-rectangle without delete old one when click in first column
                        // Todo: re-invalidate the equation, for now just comment out. Haven't seen any bug after, yet.
//                        if (left < right &&
//                                left < getWidth() &&
//                                top < getHeight() &&
...
//                        }
    ...
    private void init() {
        ...
        mDefaultEventColor = Color.parseColor("#59dbe0");
        // Set default empty event color.
        mNewEventColor = Color.parseColor("#59dbe0");
...
//            canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding / 2, isToday ? mTodayHeaderTextPaint : mHeaderTextPaint);
    ...
    private void drawEmptyImage(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        if (mNewEventIconDrawable == null)
            mNewEventIconDrawable = getResources().getDrawable(R.drawable.ic_add);
